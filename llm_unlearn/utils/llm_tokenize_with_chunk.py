...
def tokenize(
    ...
    def tokenize_function(examples):
        ...
        if "labels" in examples:
            for i, (t, l) in enumerate(zip(examples['text'], examples['labels'])):
                pre_len = tokenizer(t.replace(l, ''), return_tensors='pt').input_ids.shape[-1]
                output["labels"][i][:pre_len] = tokenizer.pad_token_id
